...
def test_abtest_writes_reports(tmp_path):
    ...
    assert report["regime_gate"]["closed"] == 0 and report["min_market_heat"] == 3.0
    assert report["insider_exit"]["closed"] == 0 and report["max_early_seller_share"] == 0.35
    md = format_report(report)
    assert "Referenz" in md and "Dichte-Kappe" in md and "Recycle-Leiter" in md
    assert "Regime-Gate" in md and "Insider-Exit" in md
